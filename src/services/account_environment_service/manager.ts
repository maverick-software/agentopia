...
`;
    ...
    async deprovisionToolbox(toolboxId: string, userId?: string): Promise<{ success: boolean; message?: string; finalStatus?: AccountToolEnvironmentStatusEnum }> {
        ...
        try {
            ...
            // Instead of updating to deprovisioned, we will now delete the record.
            ...
            await this._deleteToolboxEnvironmentRecord(toolboxId);
            console.log(`Toolbox ${toolboxId} record deleted from DB.`);
            return { success: true, finalStatus: 'deprovisioned' }; // 'deprovisioned' conceptually, as it's gone
            ...
            if (error instanceof DigitalOceanResourceNotFoundError) {
                ...
                // Proceed to delete the record in DB if DO says not found
                // const updates: AccountToolEnvironmentUpdate = {
                //     status: 'deprovisioned' as AccountToolEnvironmentStatusEnum, // Cast for enum
                //     do_droplet_id: null,
                //     public_ip_address: null,
                ...
                await this._deleteToolboxEnvironmentRecord(toolboxId)
                    .catch(deleteErr => console.error('Failed to delete Toolbox record after DO not found:', deleteErr));
                console.log(`Toolbox ${toolboxId} record deleted from DB after DO resource not found.`);
                return { success: true, message: 'Droplet not found on provider, record deleted.', finalStatus: 'deprovisioned' };
            }
    ...
    // --- Private Helper for Deleting Toolbox Record ---
    private async _deleteToolboxEnvironmentRecord(toolboxId: string): Promise<void> {
        console.log(`Deleting Toolbox environment record ${toolboxId} from database.`);
        ...
        if (error) {
            console.error(`Error deleting Toolbox environment record ${toolboxId}:`, error);
            // Depending on desired behavior, you might want to throw this error
            // or handle it more gracefully if the deprovisioning can still be considered 'complete'
            // For now, we log and it will be caught by the main deprovisionToolbox error handler if it bubbles up.
            throw new Error(`Failed to delete Toolbox environment record: ${error.message}`);
        }
        console.log(`Successfully deleted Toolbox environment record ${toolboxId}.`);
    }
