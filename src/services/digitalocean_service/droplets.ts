...
export async function createDigitalOceanDroplet(
  ...
  const response = await callWithRetry(async () => {
    ...
    // Using createDroplets, expecting a response containing a 'droplets' array.
    return doClient.droplet.createDroplets(createRequest);
  });

  // Check if response.data.droplets is an array and has at least one droplet.
  if (response?.data && Array.isArray(response.data.droplets) && response.data.droplets.length > 0) {
    return response.data.droplets[0] as Droplet; // Return the first droplet from the data property
  }
  
  // If the expected structure isn't found, log the actual response and throw an error.
  console.error("Unexpected response structure from createDroplets. Expected response.data.droplets array. Received:", response);
...
export async function getDigitalOceanDroplet(dropletId: number): Promise<Droplet> {
  ...
  // Check if response.data.droplet exists
  if (response?.data?.droplet) {
     return response.data.droplet as Droplet;
  }
  // Log the actual response structure for better debugging if parsing fails
  console.error("Unexpected response structure from getDroplet. Expected response.data.droplet. Received:", response);
