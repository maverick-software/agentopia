...
// import type { Region, Size } from 'https://esm.sh/dots-wrapper@3.11.17'; // Commented out for testing
...
export async function listAvailableRegions(): Promise<DORegion[]> {
  ...
  // This is a common pattern: response has a 'data' object, which then has the array.
  if (response && response.data && Array.isArray(response.data.regions)) {
    return response.data.regions.map((r: any) => ({
     ...
     return response.regions.map((r: any) => ({
      slug: r.slug,
      name: r.name,
      available: r.available,
      features: r.features,
...
export async function listAvailableSizes(): Promise<DOSize[]> {
  ...
  if (response && response.data && Array.isArray(response.data.sizes)) {
    return response.data.sizes.map((s: any) => ({
      slug: s.slug,
      memory: s.memory,
      vcpus: s.vcpus,
      disk: s.disk,
    ...
    return response.sizes.map((s: any) => ({
      slug: s.slug,
      memory: s.memory,
      vcpus: s.vcpus,
      disk: s.disk,
