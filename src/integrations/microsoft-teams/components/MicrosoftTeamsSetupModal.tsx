...
import { Loader2, MessageSquare, Users, Calendar, CheckCircle, AlertCircle, ExternalLink } from 'lucide-react';
import { supabase } from '@/lib/supabase';
import { toast } from 'react-hot-toast';

interface MicrosoftTeamsSetupModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
const SCOPE_DESCRIPTIONS = {
  'https://graph.microsoft.com/Chat.ReadWrite': 'Send and read chat messages',
  'https://graph.microsoft.com/Team.ReadBasic.All': 'Read basic team information',
  'https://graph.microsoft.com/Channel.ReadBasic.All': 'Read basic channel information',
  'https://graph.microsoft.com/OnlineMeetings.ReadWrite': 'Create and manage online meetings',
  'https://graph.microsoft.com/User.Read': 'Read your profile information'
};

export function MicrosoftTeamsSetupModal({ isOpen, onClose }: MicrosoftTeamsSetupModalProps) {
  ...
  const checkConnectionStatus = async () => {
    try {
      ...
      console.error('Error checking Teams connection status:', error);
      setConnectionStatus({ 
        isConnected: false, 
  ...
  const handleConnect = async () => {
    try {
      ...
      sessionStorage.setItem('teams_pkce_verifier', codeVerifier);
      sessionStorage.setItem('teams_user_id', user.id);

      // Get Microsoft Teams OAuth configuration
      const { data: provider, error: providerError } = await supabase
        .from('service_providers')
      ...
      const params = new URLSearchParams({
        ...
        state: `teams_${user.id}_${Date.now()}`,
      ...
      console.error('Error initiating Teams OAuth:', error);
      toast.error(error instanceof Error ? error.message : 'Failed to connect to Microsoft Teams');
      setIsLoading(false);
    }
  };
  ...
  const handleDisconnect = async () => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to disconnect Microsoft Teams');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto p-0">
        <DialogHeader className="px-6 pt-6 pb-4">
          ...
          <DialogDescription className="text-muted-foreground">
            Connect your Microsoft Teams account to enable messaging and meeting features for your agent.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="px-6 space-y-6">
              ...
              {connectionStatus.isConnected ? (
                <Alert className="border-green-200 bg-green-50 dark:bg-green-950 dark:border-green-800">
                  ...
                  <AlertDescription className="text-green-800 dark:text-green-200">
                    <div className="font-medium">Microsoft Teams is connected!</div>
                ...
                <Alert className="border-blue-200 bg-blue-50 dark:bg-blue-950 dark:border-blue-800">
                  ...
                  <AlertDescription className="text-blue-800 dark:text-blue-200">
                    <div className="font-medium">Microsoft Teams is not connected</div>
                    <div className="text-sm mt-1">
                      Connect your Microsoft Teams account to enable messaging and meeting features.
                    </div>
                  </AlertDescription>
                </Alert>
              )}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">What you can do with Microsoft Teams</CardTitle>
              <CardDescription>
                Enable these capabilities for your agent by connecting Microsoft Teams.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg border">
                  ...
                  <span className="text-sm font-medium">Send and read messages</span>
                </div>
                <div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg border">
                  <Users className="h-5 w-5 text-green-500 flex-shrink-0" />
                  <span className="text-sm font-medium">Access team information</span>
                </div>
                <div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg border">
                  <Calendar className="h-5 w-5 text-purple-500 flex-shrink-0" />
                  <span className="text-sm font-medium">Create meetings</span>
                </div>
                <div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg border">
                  <ExternalLink className="h-5 w-5 text-orange-500 flex-shrink-0" />
