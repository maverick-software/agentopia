...
function DiscordConnectComponent({ 
  ...
  return (
    <> {/* NEW: Top-level Fragment to wrap main div and modals */}
      <div className={`space-y-4 ${className}`}> 
        {!showFullUI ? (
          // --- Initial Bot Token Input View ---
          <>
            {/* REMOVED: Connect to Discord Title */}
            {/* <h3 className="text-lg font-medium text-white">Connect to Discord</h3> */}
            <div> 
                 <label htmlFor="initialBotToken" className="block text-sm font-medium text-gray-300 mb-1">Bot Token *</label>
                 <div className="flex space-x-2">
                     <input
                         id="initialBotToken"
                         type="password" // Keep it password type for initial entry too
                         value={initialBotKey}
          ...
          // --- Full Connection Management View ---
          // Use Fragment to wrap existing content
          <> 
            {/* Top row: Title and Status/Toggle Area */}
            <div className="flex justify-between items-center">
              {/* RESTORED: Title */}
              <h3 className="text-lg font-medium text-white">Discord Configuration</h3>
              {/* Combined Status/Toggle Area */}
              <div className="flex items-center space-x-2">
                 {renderWorkerStatus()} {/* Moved here */}
                 {/* Simplified Activation Toggle Button */}
                 <button 
                     type="button"
                     id="activationToggle" // ID kept for potential future use
                     role="switch"
                 ...
                 >
                     ...
                     <span
                         className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform duration-200 ease-in-out ${workerStatus === 'active' ? 'translate-x-6' : 'translate-x-1'}`}
                     />
            ...
            {/* Removed original Status & Activation Controls div */}
            {/* Removed error display <p> - might be better placed elsewhere or handled differently */}
            {/* {error && <p className="text-sm text-red-400">Error: {error}</p>} */}

            {/* Management Buttons - Now correctly placed */}
            ...
            {/* Warning Text (Moved below buttons, styled gray) */}
          ...
          </> // End Full UI Fragment
        )} 
      </div> {/* End of main component div */}

      {/* --- Modals Rendered OUTSIDE the main div --- */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 rounded-lg p-6 w-full max-w-lg space-y-4">
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={handleCloseCredentialsModal} className="text-gray-400 hover:text-white"><X size={20} /></button>
            </div>
            ...
            {/* App ID - Now with visibility toggle */}
            <div>
         ...
         <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
           <div className="bg-gray-800 rounded-lg p-6 w-full max-w-lg space-y-4">
              <div className="flex justify-between items-center mb-4">
                   ...
                   <button onClick={handleCloseSettingsModal} className="text-gray-400 hover:text-white"><X size={20} /></button>
               </div>
              {/* Guild Selection */} 
              <div> <label htmlFor="modalGuildSelect" className="block text-sm font-medium text-gray-300 mb-1">Active Discord Server</label> <select id="modalGuildSelect" value={currentGuildId ?? ''} onChange={(e) => onConnectionChange('guild_id', e.target.value || null)} disabled={isWorkerBusy} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"> <option value="">-- Select a Server --</option> {allGuilds && allGuilds.map((guild) => ( <option key={guild.id} value={guild.id}> {guild.name} ({guild.id}) </option> ))} </select> </div>
              {/* Inactivity Timeout Dropdown */} 
              <div> <label htmlFor="modalInactivityTimeout" className="block text-sm font-medium text-gray-300 mb-1">Inactivity Timeout</label> <select id="modalInactivityTimeout" value={localTimeout} onChange={handleTimeoutChange} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"> {TIMEOUT_OPTIONS.map(opt => ( <option key={opt.value} value={opt.value}>{opt.label}</option> ))} </select> <p className="mt-1 text-xs text-gray-400">Time before the agent worker stops due to inactivity.</p> </div>
              {/* Interaction Endpoint URL */} 
              <div> <label className="block text-sm font-medium text-gray-300 mb-1">Interaction Endpoint URL</label> <div className="flex items-center space-x-2"> <input type="text" readOnly value={`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/discord-interaction-handler`} className="flex-grow px-3 py-2 bg-gray-800 border border-gray-700 rounded-md text-gray-400"/> <button type="button" onClick={() => copyToClipboard(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/discord-interaction-handler`)} className="p-2 text-gray-400 hover:text-white"> {copied ? <Check size={18} className="text-green-400"/> : <Copy size={18} />} </button> </div> <p className="mt-1 text-xs text-gray-400"> Copy this URL into your Discord App settings. </p> </div>

              {/* Action Buttons */}
               <div className="flex justify-end pt-4">
                    <button type="button" onClick={handleSaveChangesFromSettingsModal} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors text-sm font-medium"> Done </button>
               </div>
           </div>
         </div>
     )}
    </> // End NEW top-level Fragment
  );
}
