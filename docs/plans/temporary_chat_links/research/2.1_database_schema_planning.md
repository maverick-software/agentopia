...
20250918000001_create_temporary_chat_links.sql

-- Migration 2: Session management  
20250918000002_create_temporary_chat_sessions.sql

-- Migration 3: Indexes and performance
20250918000003_create_temporary_chat_indexes.sql

-- Migration 4: RLS policies
20250918000004_create_temporary_chat_rls.sql

-- Migration 5: Functions and cleanup
20250918000005_create_temporary_chat_functions.sql
```
...
```sql
-- Migration: 20250918000001_create_temporary_chat_links.sql
CREATE TABLE IF NOT EXISTS temporary_chat_links (
  ...
  -- Link identification and access
  ...
  CONSTRAINT valid_token_format CHECK (link_token ~ '^[A-Za-z0-9_-]+$') -- URL-safe characters
);
```
...
CREATE TABLE IF NOT EXISTS temporary_chat_sessions (
  ...
  CONSTRAINT valid_token_format CHECK (session_token ~ '^[A-Za-z0-9_-]+$')
);
```
...
CREATE INDEX idx_temp_links_token ON temporary_chat_links(link_token) WHERE is_active = TRUE;
...
CREATE INDEX idx_temp_sessions_token ON temporary_chat_sessions(session_token) WHERE status = 'active';
...
CREATE INDEX idx_temp_sessions_rate_limit ON temporary_chat_sessions(session_token, last_message_at) 
  WHERE status = 'active';
```
...
```sql
-- Migration: 20250918000004_create_temporary_chat_rls.sql

-- Enable RLS
ALTER TABLE temporary_chat_links ENABLE ROW LEVEL SECURITY;

-- Policy 1: Users can manage their own temporary links
CREATE POLICY "users_manage_own_temp_links" ON temporary_chat_links
  FOR ALL USING (user_id = auth.uid());

-- Policy 2: Users can manage links for their agents
CREATE POLICY "users_manage_agent_temp_links" ON temporary_chat_links
  FOR ALL USING (
    agent_id IN (SELECT id FROM agents WHERE user_id = auth.uid())
  );

-- Policy 3: Service role has full access for system operations
CREATE POLICY "service_role_full_access_temp_links" ON temporary_chat_links
  FOR ALL USING (auth.role() = 'service_role');

-- Policy 4: Public read access for active, non-expired links (for validation)
CREATE POLICY "public_read_active_links" ON temporary_chat_links
  FOR SELECT USING (
    is_active = TRUE 
    AND expires_at > NOW()
    AND auth.role() = 'anon'
...
-- Policy 1: Service role only for security (no direct user access)
CREATE POLICY "service_role_only_temp_sessions" ON temporary_chat_sessions
  FOR ALL USING (auth.role() = 'service_role');

-- Policy 2: Users can view sessions for their links (for management)
CREATE POLICY "users_view_own_link_sessions" ON temporary_chat_sessions
  FOR SELECT USING (
...
```sql
-- Migration: 20250918000005_create_temporary_chat_functions.sql

-- Function: Generate secure link token
...
-- Function: Validate and get temporary chat session
