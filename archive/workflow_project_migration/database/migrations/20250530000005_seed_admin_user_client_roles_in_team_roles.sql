...
BEGIN
    RAISE NOTICE 'Starting team_roles seeding script (v3 - with team_members prerequisite).';
    ...
    LOOP
        ...
        IF auth_user_exists THEN
            ...
            LOOP
                ...
                -- Step 1: Ensure a link exists in team_members (assuming this table exists and is the FK target)
                -- This is a critical assumption. If team_members table was dropped or schema changed, this will fail.
                ...
                BEGIN
                    INSERT INTO team_members (auth_user_id, client_id, status) -- Assuming these columns
                    VALUES (admin_profile.auth_user_id, client_record.id, 'ACTIVE') -- Assuming 'ACTIVE' status
                    ON CONFLICT (auth_user_id, client_id) DO NOTHING; -- Assuming unique constraint or PK on (auth_user_id, client_id)
                ...
                EXCEPTION
                    WHEN undefined_table THEN
                        ...
                        CONTINUE; -- Skip to next client if team_members table is missing
                    WHEN others THEN
                        RAISE WARNING 'Error ensuring team_members entry for user %, client %: %', admin_profile.auth_user_id, client_record.id, SQLERRM;
                        CONTINUE; -- Skip to next client on other errors related to team_members
                END;


                -- Step 2: Get the ID of the 'Client Admin' role for the current client
        ...
        END IF;
    END LOOP;
    RAISE NOTICE 'Finished team_roles seeding script (v3).';
END $$; 