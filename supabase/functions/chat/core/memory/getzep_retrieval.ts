...
export async function searchGetZepKnowledgeGraph(
  ...
  try {
    // GUARD CLAUSE #1: Check if account graph is enabled for ANY agent of this user
    // This is a user-level setting, not agent-level
    ...
    // GUARD CLAUSE #2: Check for active GetZep connection
    ...
    if (!provider) {
      console.log('[GetZepSemantic] GetZep provider not configured - skipping');
      return [];
    }
    ...
    // EARLY EXIT: No connection = no search
    if (!connection?.vault_access_token_id) {
      console.log('[GetZepSemantic] No active GetZep connection - skipping');
      return [];
    }
    
    // GUARD CLAUSE #2: Only NOW load SDK (expensive operation)
    ...
    try {
      ...
      console.warn('[GetZepSemantic] SDK import failed; skipping retrieval:', (sdkErr as any)?.message || sdkErr);
      return [];
    }
