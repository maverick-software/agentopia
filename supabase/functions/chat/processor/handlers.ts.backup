...
export class TextMessageHandler implements MessageHandler {
  ...
  async handle(message: AdvancedChatMessage, context: ProcessingContext) {
    ...
    if (context.agent_id) {
      ...
      // ✅ NEW: Append system_prompt_override from sessionContext (for temporary chats)
      ...
      if (sessionContext?.system_prompt_override) {
        console.log('[TextMessageHandler] Appending system_prompt_override from sessionContext');
        systemPrompt = systemPrompt 
          ? `${systemPrompt}\n\n=== TEMPORARY CHAT INSTRUCTIONS ===\n${sessionContext.system_prompt_override}\n=== END TEMPORARY CHAT INSTRUCTIONS ===`
          : sessionContext.system_prompt_override;
      }
      
      // ✅ NEW: Add chat_intent context if present
      if (sessionContext?.chat_intent) {
        console.log('[TextMessageHandler] Adding chat_intent context from sessionContext');
    ...
    if (conversationId && context.agent_id) {
      try {
        ...
        if (memoryContext && memoryContext.summary) {
          ...
          console.log(`[TextMessageHandler] 📝 Adding last ${recentMessages.length} messages for immediate context (hybrid approach)`);
    ...
    if (toolCalls.length > 0 || mcpRetryAttempts > 0) {
      ...
      // CRITICAL: Convert tool results to user messages for reflection
      // The LLM needs to see the tool results to generate a proper response
      // Convert 'tool' role messages to 'user' role with clear formatting
      ...
      if (router && useRouter && context.agent_id) {
        ...
        console.log(`[TextMessageHandler] ✅ Reflection complete, response length: ${resp2.text?.length || 0}`);
      } else {
        completion = await this.openai.chat.completions.create({
          model: 'gpt-4',
        ...
        console.log(`[TextMessageHandler] ✅ Reflection complete, response length: ${completion.choices?.[0]?.message?.content?.length || 0}`);
      }
    }
