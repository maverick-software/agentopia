import { backOff } from 'https://esm.sh/exponential-backoff@3.1.2';
import { DigitalOceanServiceError, DigitalOceanApiError, DigitalOceanAuthenticationError, DigitalOceanResourceNotFoundError, DigitalOceanRateLimitError } from './errors.ts';
 ...
 */ export async function callWithRetry(apiCall, options) {
  ...
  try {
    return await backOff(apiCall, {
      ...
      retry: (e, attemptNumber)=>{
        console.warn(`DigitalOcean API call attempt ${attemptNumber} failed. Error: ${e?.message || 'Unknown error'}. Analyzing for retry...`);
    ...
    console.error(`DigitalOcean API call failed definitively: ${originalErrorMessage}`, {
    ...
    throw new DigitalOceanServiceError(`Failed DigitalOcean API operation: ${originalErrorMessage}`, error);
  }
}
