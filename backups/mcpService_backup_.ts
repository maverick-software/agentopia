...
export class MCPService {
    ...
    try {
      ...
      const servers: MCPServer[] = (data || []).map(instance => ({
        ...
        status: {
          ...
          uptime: instance.status_on_toolbox === 'installed' ? Math.floor((Date.now() - new Date(instance.created_at).getTime()) / 1000) : 0,
  ...
  async deployServer(config: MCPDeploymentConfig): Promise<MCPDeploymentStatus> {
    try {
      ...
      // Get user's first account tool environment (or create one)
      let { data: environments, error: environmentError } = await supabase
      ...
      if (!environments || environments.length === 0) {
        // Create a default environment for MCP servers
      ...
      const { data: instance, error: instanceError } = await supabase
        .from('account_tool_instances')
        .insert({
          account_tool_environment_id: environmentId,
          tool_catalog_id: '00000000-0000-0000-0000-000000000001', // We'll need to create a generic MCP tool entry
          instance_name_on_toolbox: config.name,
          status_on_toolbox: 'installed',
          mcp_server_type: config.server_type,
          mcp_endpoint_path: config.endpoint_url || '/mcp',
          mcp_transport_type: config.transport_type || 'stdio',
